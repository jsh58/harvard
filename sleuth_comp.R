...
cat('  TPR:', ifelse(tp.se + fn.se > 0, tp.se/(tp.se + fn.se), 'NA'), '\n')
cat('  FPR:', ifelse(fp.se + tn.se > 0, fp.se/(fp.se + tn.se), 'NA'), '\n')
cat('  recall:', ifelse(tp.se + fn.se > 0, tp.se/(tp.se + fn.se), 'NA'), '\n')
cat('  precision:', ifelse(tp.se + fp.se > 0, tp.se/(tp.se + fp.se), 'NA'), '\n')
...
cat('  TPR:', ifelse(tp.pe + fn.pe > 0, tp.pe/(tp.pe + fn.pe), 'NA'), '\n')
cat('  FPR:', ifelse(fp.pe + tn.pe > 0, fp.pe/(fp.pe + tn.pe), 'NA'), '\n')
cat('  recall:', ifelse(tp.pe + fn.pe > 0, tp.pe/(tp.pe + fn.pe), 'NA'), '\n')
cat('  precision:', ifelse(tp.pe + fp.pe > 0, tp.pe/(tp.pe + fp.pe), 'NA'), '\n')
